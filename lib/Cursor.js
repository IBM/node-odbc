class Cursor {
  ...
  static CUSROR_CLOSED_ERROR = 'Cannot fetch, the cursor has already been closed!';
  ...
  fetch(cb) {
    ...
    if (typeof callback !== 'function') {
      if (!this.odbcCursor) {
        return new Promise((resolve, reject) => {
          reject('Cannot fetch, the cursor has already been closed!');
        })
      }
    ...
    if (!this.odbcCursor) {
      return callback(new Error(Cursor.CUSROR_CLOSED_ERROR));
    }
  ...
  close(callback = undefined) {
    ...
    // ...or callback
    return this.odbcCursor.close((error) => {
      if (!error) {
        this.odbcCursor = null;
      }
      callback(error);
