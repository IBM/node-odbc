    ...
    return new Promise((resolve, reject) => {
      odbc.connect(connectionConfig, (error, nativeConnection) => {
        if (error) {
          ...
          // with the error that is then either handled by any waiting work, or // is silently swallowed if no work is waiting. This is so that a bad
          ...
          // This reject is swallowed in all cases, and is simply to end the
          // Promise that was generated by generateConnectPromise.
          reject(error);
          return;
        }
