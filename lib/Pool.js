...
class Pool {
  ...
  async init(callback = undefined) {
    ...
    console.log('pool.init() was called, but the pool was already initialized.');
    return undefined;
  }
  ...
  async increasePoolSize(count) {
    console.log(`ips connection string is CONNECTION STRING ${this.connectionString}`);
    return new Promise((resolve, reject) => {
  ...
  static async generateConnections(connectionString, count, callback) {
    console.log(`CALLING ODBC CONNECTION WITH CONNECTION STRING ${connectionString}`);
    // promise...
    if (typeof callback === 'undefined') {
      return new Promise((resolve, reject) => {
