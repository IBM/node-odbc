...
void ODBC::Init(v8::Handle<Object> exports) {
  ...
  Nan::SetPrototypeTemplate(constructor_template, Nan::New<String>("SQL_CLOSE").ToLocalChecked(), Nan::New<Number>(SQL_CLOSE), constant_attributes);
  Nan::SetPrototypeTemplate(constructor_template, Nan::New<String>("SQL_DROP").ToLocalChecked(), Nan::New<Number>(SQL_DROP), constant_attributes);
  Nan::SetPrototypeTemplate(constructor_template, Nan::New<String>("SQL_UNBIND").ToLocalChecked(), Nan::New<Number>(SQL_UNBIND), constant_attributes);
  Nan::SetPrototypeTemplate(constructor_template, Nan::New<String>("SQL_RESET_PARAMS").ToLocalChecked(), Nan::New<Number>(SQL_RESET_PARAMS), constant_attributes);
  Nan::SetPrototypeTemplate(constructor_template, Nan::New<String>("SQL_DESTROY").ToLocalChecked(), Nan::New<Number>(SQL_DESTROY), constant_attributes);
  Nan::SetPrototypeTemplate(constructor_template, Nan::New<String>("FETCH_ARRAY").ToLocalChecked(), Nan::New<Number>(FETCH_ARRAY), constant_attributes);
  ...
  Nan::GetFunction(constructor_template);
  Nan::Set(exports, Nan::New("ODBC").ToLocalChecked(), Nan::GetFunction(constructor_template).ToLocalChecked());
...
void ODBC::UV_AfterCreateConnection(uv_work_t* req, int status) {
  ...
  if (!SQL_SUCCEEDED(data->result)) {
    ...
    Local<Object> js_result = Nan::NewInstance(Nan::New<Function>(ODBCConnection::constructor)2, info);
...
NAN_METHOD(ODBC::CreateConnectionSync) {
  ...
  Local<Object> js_result = Nan::NewInstance(Nan::New<Function>(ODBCConnection::constructor)2, params);
    ...
    case SQL_TIMESTAMP : {
      ...
      if (len == SQL_NULL_DATA) {
        ...
        if (strptime((char *) buffer, "%Y-%m-%d %H:%M:%S", &timeInfo)) {
          ...
          return scope.Escape(Nan::New<Date>(double(mktime(&timeInfo).ToLocalChecked()) * 1000));
        }
        else {
      ...
      if (len == SQL_NULL_DATA) {
        ...
        timeInfo.tm_isdst = -1;
#ifdef TIMEGM
        return scope.Escape(Nan::New<Date>((double(timegm(&timeInfo).ToLocalChecked()) * 1000)
                          + (odbcTime.fraction / 1000000)));
#else
        return scope.Escape(Nan::New<Date>((double(timelocal(&timeInfo).ToLocalChecked()) * 1000)
                          + (odbcTime.fraction / 1000000)));
#endif
...
Local<Object> ODBC::GetRecordTuple ( SQLHSTMT hStmt, Column* columns, 
  ...
  for(int i = 0; i < *colCount; i++) {
#ifdef UNICODE
    Nan::Set(tuple,  Nan::New((uint16_t *) columns[i].name),
                GetColumnValue( hStmt, columns[i], buffer, bufferLength));
#else
    Nan::Set(tuple,  Nan::New((const char *) columns[i].name),
                GetColumnValue( hStmt, columns[i], buffer, bufferLength));
#endif
  }
...
Parameter* ODBC::GetParametersFromArray (Local<Array> values, int *paramCount) {
  ...
  *paramCount = values->Length();
  ...
  for (int i = 0; i < *paramCount; i++) {
    Local<Value> value = NanGet(values, i);
    ...
    if (value->IsString()) {
      ...
      int64_t  *number = new int64_t(NanTo<int64_t>(value));
      ...
      double *number   = new double(NanTo<int64_t>(value));
      ...
      bool *boolean = new bool(NanTo<bool>(value));
...
Local<Object> ODBC::GetSQLError (SQLSMALLINT handleType, SQLHANDLE handle, char* message) {
  ...
  for (i = 0; i < statusRecCount; i++){
    ...
    if (SQL_SUCCEEDED(ret)) {
      ...
      if (i == 0) {
        ...
        Nan::Set(objError, Nan::New("error").ToLocalChecked(), Nan::New(message));
#ifdef UNICODE
        Nan::SetPrototype(objError, Exception::Error(Nan::New((uint16_t *)errorMessage)));
        Nan::Set(objError, Nan::New("message").ToLocalChecked(), Nan::New((uint16_t *)errorMessage));
        Nan::Set(objError, Nan::New("state").ToLocalChecked(), Nan::New((uint16_t *)errorSQLState));
#else
        Nan::SetPrototype(objError, Exception::Error(Nan::New(errorMessage)));
        Nan::Set(objError, Nan::New("message").ToLocalChecked(), Nan::New(errorMessage));
        Nan::Set(objError, Nan::New("state").ToLocalChecked(), Nan::New(errorSQLState));
#endif
      }
...
#ifdef UNICODE
      Nan::Set(subError, Nan::New("message").ToLocalChecked(), Nan::New((uint16_t *)errorMessage));
      Nan::Set(subError, Nan::New("state").ToLocalChecked(), Nan::New((uint16_t *)errorSQLState));
#else
      Nan::Set(subError, Nan::New("message").ToLocalChecked(), Nan::New(errorMessage));
      Nan::Set(subError, Nan::New("state").ToLocalChecked(), Nan::New(errorSQLState));
#endif
  ...
  if (statusRecCount == 0) {
    ...
    Nan::Set(objError, Nan::New("error").ToLocalChecked(), Nan::New(message));
    Nan::SetPrototype(objError, Exception::Error(Nan::New(message)));
...
extern "C" void init(v8::Handle<Object> exports) {
  ...
  Nan::Set(exports, Nan::New("loadODBCLibrary").ToLocalChecked(),
        Nan::GetFunction(Nan::New<FunctionTemplate>(ODBC::LoadODBCLibrary)));
#endif
