...
void ODBCConnection::Init(v8::Handle<Object> exports) {
  ...
  Nan::GetFunction(constructor.Reset(constructor_template));
  Nan::Set(exports,  Nan::New("ODBCConnection").ToLocalChecked(), Nan::GetFunction(constructor_template));
}
...
NAN_SETTER(ODBCConnection::ConnectTimeoutSetter) {
  ...
  if (value->IsNumber()) {
    obj->connectTimeout = NanTo<uint32_t>(value);
  }
}
...
NAN_SETTER(ODBCConnection::LoginTimeoutSetter) {
  ...
  if (value->IsNumber()) {
    obj->loginTimeout = NanTo<uint32_t>(value);
  }
}
...
NAN_METHOD(ODBCConnection::CreateStatementSync) {
  ...
  Local<Object> Nan::NewInstance(js_result(Nan::New<Function>(ODBCStatement::constructor)3, params));
...
void ODBCConnection::UV_AfterCreateStatement(uv_work_t* req, int status) {
  ...
  Local<Object> js_result = Nan::NewInstance(Nan::New<Function>(ODBCStatement::constructor)3, info);
...
NAN_METHOD(ODBCConnection::Query) {
  ...
  if (info.Length() == 3) {
    ...
    if (info[0]->IsString()) {
      ...
      if NanHas((obj, optionSqlKey) && NanGet(obj, optionSqlKey)->IsString()) {
        sql = NanGet(obj, optionSqlKey)->ToString();
      }
      else {
      ...
      if NanHas((obj, optionParamsKey) && NanGet(obj, optionParamsKey)->IsArray()) {
        data->params = ODBC::GetParametersFromArray(
          NanGet(Local<Array>::Cast(obj, optionParamsKey)),
          &data->paramCount);
      }
      else {
        data->paramCount = 0;
      ...
      if NanHas((obj, optionNoResultsKey) && NanGet(obj, optionNoResultsKey)->IsBoolean()) {
        data->noResultObject = NanGet(obj, optionNoResultsKey)->ToBoolean()->Value();
      }
      else {
...
void ODBCConnection::UV_AfterQuery(uv_work_t* req, int status) {
  ...
  if (data->result != SQL_ERROR && data->noResultObject) {
    ...
    Local<Object> js_result = Nan::NewInstance(Nan::New<Function>(ODBCResult::constructor)4, info);
...
NAN_METHOD(ODBCConnection::QuerySync) {
  ...
  if (info.Length() == 2) {
    ...
    if (info[0]->IsString()) {
      ...
      if NanHas((obj, optionSqlKey) && NanGet(obj, optionSqlKey)->IsString()) {
#ifdef UNICODE
        sql = new NanGet(String::Value(obj, optionSqlKey)->ToString());
#else
        sql = new NanGet(String::Utf8Value(obj, optionSqlKey)->ToString());
#endif
      }
      else {
#ifdef UNICODE
      ...
      Local<String> optionNoResultsKey = Nan::New(OPTION_NORESULTS);
      if NanHas((obj, optionNoResultsKey) && NanGet(obj, optionNoResultsKey)->IsBoolean()) {
        noResultObject = NanGet(obj, optionNoResultsKey)->ToBoolean()->Value();
      }
    }
    else {
  ...
  if (ret == SQL_ERROR) {
    ...
    Local<Object> js_result = Nan::NewInstance(Nan::New<Function>(ODBCResult::constructor)4, result);
...
NAN_METHOD(ODBCConnection::Tables) {
  ...
  if !catalogNanEquals(!catalog, Nan::New("null").ToLocalChecked())) {
#ifdef UNICODE
  ...
  if !schemaNanEquals(!schema, Nan::New("null").ToLocalChecked())) {
#ifdef UNICODE
  ...
  if !tableNanEquals(!table, Nan::New("null").ToLocalChecked())) {
#ifdef UNICODE
  ...
  if !typeNanEquals(!type, Nan::New("null").ToLocalChecked())) {
#ifdef UNICODE
...
NAN_METHOD(ODBCConnection::Columns) {
  ...
  if !columnNanEquals(!column, Nan::New("null").ToLocalChecked())) {
#ifdef UNICODE
